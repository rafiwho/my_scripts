...
@("$PID") +  (Get-Content "$PSScriptRoot\running.in" -ErrorAction SilentlyContinue) | Set-Content "$PSScriptRoot\running.in" -ErrorAction SilentlyContinue
...
while(-not $p.HasExited -And $p -ne $null) {
    ...
    if($OutputSize -gt $MaxOutputSize) {
        ...
        Clear-Content "$PSScriptRoot\running.in"
        exit
    }
